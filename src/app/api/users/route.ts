...
export async function GET(req: Request) {
  try {
    ...
    const users = await UserService.listAll(organizationId)
...
export async function POST(req: Request) {
  try {
    ...
    const currentRole = await getUserRole(userSession.email!, organizationId)
    ...
    const newUser = await UserRepository.create(organizationId, {
      id: authUser.user.id,
      name,
      email,
      role,
